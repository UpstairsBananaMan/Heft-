...
export default function RateJob() {
  ...
  async function submit() {
    const row = job.data;
    if (!row || !profile || !row.driver_id) return;
    setPending(true);
    setError("");
    try {
  ...
  return (
    <Screen title="Rate driver" back>
      <Text className="mb-4 text-sm text-steel">One rating per person on this job.</Text>
      <View className="mb-4 flex-row gap-2">
        {[1, 2, 3, 4, 5].map((value) => (
          <Pressable key={value} onPress={() => setStars(value)} className={`min-h-[48px] min-w-[48px] items-center justify-center ${value <= stars ? "bg-amber" : "bg-white"} border border-line`}>
