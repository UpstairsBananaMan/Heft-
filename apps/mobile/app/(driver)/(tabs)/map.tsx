  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.paper }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
        ...
        {rows.map((job) => (
          <JobOffer key={job.id} job={job} onAccept={() => accept(job)} onOpen={() => router.push(`/job/${job.id}`)} />
        ))}
      </ScrollView>
    </View>
  );
...
function JobOffer({ job, onAccept, onOpen }: { job: Job; onAccept: () => void; onOpen: () => void }) {
  const away = haversineMiles(PENSACOLA_CENTER.lat, PENSACOLA_CENTER.lng, job.pickup_lat, job.pickup_lng);
  const trip = Number(job.distance_miles ?? 0);
  const payout = job.driver_payout_cents ?? 0;
  const perMile = trip + away > 0 ? payout / 100 / (trip + away) : 0;
    ...
    <View accessibilityLabel={jobA11yLabel(job)} style={{ backgroundColor: C.white, borderRadius: 18, padding: 16, marginBottom: 12 }}>
      <Pressable onPress={onOpen}>
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <Text style={{ color: C.steel, fontFamily: font.medium, fontSize: 14 }}>Your payout</Text>
        ...
        <Text style={{ fontFamily: font.display, fontSize: 34, color: C.ink }}>{formatUsd(payout)}</Text>
        <Meta icon={<Route color={C.steel} size={16} />} text={`${trip.toFixed(1)} mi trip · ${away.toFixed(1)} mi away`} />
        <Meta icon={<Sofa color={C.steel} size={16} />} text={`${job.item_description} · ${job.size_category}`} />
        <Meta icon={<Footprints color={C.steel} size={16} />} text={stairs > 0 ? `${stairs} flight at pickup` : "No stairs"} />
        <Meta icon={<Users color={C.steel} size={16} />} text={job.needs_helper ? "Needs a second person" : "Customer will help"} />
      ...
      <View style={{ marginTop: 12 }}>
        <HoldToAccept onAccept={onAccept} />
      </View>
    </View>
  );
}
