...
import { Map, UserRound, Wallet } from "lucide-react-native";
import { C, font } from "../../../src/components/v2";

function Pill({ focused, children }: { focused: boolean; children: ReactNode }) {
  return (
    <View style={{ width: 60, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center", backgroundColor: focused ? C.amber100 : "transparent" }}>
      {children}
    </View>
  );
}
    ...
    >
        ...
        options={{
          ...
          tabBarIcon: ({ color, focused }) => (
            <Pill focused={focused}>
              <Map color={color} size={22} strokeWidth={focused ? 2.1 : 1.75} />
            </Pill>
          ),
        }}
      />
      <Tabs.Screen
        ...
        options={{
          ...
          tabBarIcon: ({ color, focused }) => (
            <Pill focused={focused}>
              <Wallet color={color} size={22} strokeWidth={focused ? 2.1 : 1.75} />
            </Pill>
          ),
        }}
      />
